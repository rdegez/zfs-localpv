...
but restoring it requires the volume's original key, which the auto and KMS
modes delete together with the volume. Until then, back up such volumes at the
application level.
...
- Key material is never written to the ZFSVolume CR or the node disk, and is fed
  to `zfs` over stdin. **Secret-backed modes (1 and 3) store the key in a
...
- Velero backups of encrypted volumes are **not** encrypted — see *Backup and
